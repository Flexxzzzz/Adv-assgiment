...
class Product:
    ...
    def tax_for(self, amount):
        """Return the tax for an amount of this product."""
        rate = FOOD_TAX_RATE if self.category == "food" else TAX_RATE
        return amount * rate
...
def build_products():
    """Convert the legacy product data into domain objects."""
    return [Product(name, price, category) for name, price, category in PRODUCTS]


def build_orders(products):
    """Convert the legacy order data into composed domain objects."""
    orders = []
    for customer_name, tier, raw_items in ORDERS:
        customer = TIER_CLASSES[tier](customer_name)
