...
class Product:
    def __init__(self, name, price, category):
        if not name or price < 0 or category not in ("electronics", "stationery", FOOD_CATEGORY):
...
class Customer:
    ...
    def discount_rate(self, subtotal):
        return self.large_discount if subtotal > DISCOUNT_THRESHOLD else self.small_discount
...
class Order:
    def __init__(self, customer, items):
        if not isinstance(customer, Customer) or not items or not all(isinstance(item, OrderItem) for item in items):
    ...
    def discount(self):
        ...
        amount = subtotal * self.customer.discount_rate(subtotal)
        if sum(item.quantity for item in self.items) >= BULK_QTY_THRESHOLD:
            amount += subtotal * BULK_DISCOUNT_RATE
        return amount
    ...
    def receipt(self):
        lines = ["Receipt for " + self.customer.name + " (" + self.customer.tier + ")", "-" * RECEIPT_WIDTH]
...
def refactored_main():
    ...
    for name, tier, item_data in ORDERS:
        customer = CUSTOMER_CLASSES[tier](name)
        order = Order(customer, [OrderItem(products[index], quantity) for index, quantity in item_data])
        print(order.receipt())
